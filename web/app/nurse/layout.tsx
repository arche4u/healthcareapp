...
import { Activity, Clipboard, Users, Settings, Bell, ChevronRight } from "lucide-react";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/Button";

const navItems = [
  { name: "Triage Queue", href: "/nurse", icon: Activity },
  { name: "Patients", href: "/nurse/patients", icon: Users },
  { name: "Vitals", href: "/nurse/vitals", icon: Clipboard },
    ...
    <div className="flex h-screen bg-background">
      ...
      <aside className="w-64 border-r border-border bg-card hidden md:flex flex-col">
        <div className="p-6">
          ...
          <p className="text-xs text-muted-foreground mt-1">Nurse Dashboard</p>
        </div>
        ...
        <div className="p-4 border-t border-border">
          <Link href="/auth/signin" className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-accent transition-colors">
            ...
            <div className="flex-1 text-left">
              <p className="font-medium truncate">{user?.full_name || "Nurse"}</p>
              <p className="text-xs text-muted-foreground capitalize">Nursing Staff</p>
            </div>
          </Link>
        </div>
      </aside>
