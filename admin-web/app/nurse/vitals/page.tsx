          ...
          res.encounters.map(async (enc: any) => {
            ...
            try {
              const [pRes, cRes] = await Promise.all([
                ...
                appointmentsAPI.getClinicalData(enc.id)
              ]);
              return { ...enc, patient: pRes.patient, observations: cRes.observations || [] };
            } catch {
              const pRes = await identityAPI.getPatientBundle(patientId).catch(() => ({ patient: null }));
              return { ...enc, patient: pRes.patient, observations: [] };
            }
          })
        );
        setQueue(enriched);
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {queue.map((encounter) => {
            ...
            // Extract vitals from observations (if already triaged)
            const obs = encounter.observations || [];
            const getObs = (name: string) => obs.find((o: any) => o.code?.text?.toLowerCase().includes(name.toLowerCase()))?.valueString;
            const hrVal = getObs("Heart Rate") || getObs("hr");
            const bpVal = getObs("Blood Pressure") || getObs("bp");
            const tempVal = getObs("Temp") || getObs("Temperature");
            const spo2Val = getObs("SpO2") || getObs("spo2");
            const vitalsTaken = !!(hrVal || bpVal || tempVal || spo2Val);
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-muted/50 p-3 rounded-lg flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{hrVal ? `${hrVal} bpm` : "-- bpm"}</p>
                      </div>
                    </div>
                    <div className="bg-muted/50 p-3 rounded-lg flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{bpVal || "--/--"}</p>
                      </div>
                    </div>
                    <div className="bg-muted/50 p-3 rounded-lg flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{tempVal ? `${tempVal} °C` : "-- °C"}</p>
                      </div>
                    </div>
                    <div className="bg-muted/50 p-3 rounded-lg flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{spo2Val ? `${spo2Val} %` : "-- %"}</p>
                      </div>
                    </div>
                  </div>
