...
import {
  ArrowLeft, Activity, FileText, Pill, Plus, CheckCircle, Brain, History, User, Mic, MicOff, Users
} from "lucide-react";
import { Button } from "@/components/ui/Button";
import { appointmentsAPI, reportsAPI, identityAPI } from "@/lib/api";
...
export default function ConsultationPage({ params }: { params: { id: string } }) {
  ...
  const loadData = async () => {
    try {
      ...
      if (result.patient?.id) {
        try {
          const famRes = await identityAPI.getPatientFamily(result.patient.id);
          setFamily(famRes.filter((f: any) => f.id !== result.patient.id));
        } catch (e) {
  ...
  const handlePrescribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fire SSE event so patient portal syncs in real-time
      fetch('/api/events', {
        method: 'POST',
  ...
  const handleVoicePrescribe = async () => {
    ...
    recognition.onend = () => { if (voicePrescribing) setVoicePrescribing(false); };
    recognition.start();
  };
              ...
              <section className="mt-6 border-t pt-6">
                ...
                <div className="space-y-4">
                  {family.map((member: any) => (
                    <div key={member.id} className="bg-card border p-4 rounded-xl shadow-sm border-l-4 border-l-blue-500">
                      <div className="flex justify-between items-center mb-2">
                        <div className="font-semibold">{member.name?.[0]?.given?.[0] || 'Unknown'}</div>
                        <div className="text-xs text-muted-foreground capitalize">{member.gender} • {member.birthDate ? Math.floor((new Date().getTime() - new Date(member.birthDate).getTime()) / 31557600000) + ' yrs' : 'Unknown'}</div>
                      </div>
                      {member.medical_issues && member.medical_issues.length > 0 ? (
                        <div className="text-sm bg-red-50 text-red-700 p-2 rounded border border-red-100">
                          <span className="font-semibold text-xs uppercase block mb-1">Known Issues:</span>
                          {member.medical_issues[0].valueString || "Various historical records"}
                        </div>
                      ) : (
                        <div className="text-sm text-muted-foreground italic">No known medical issues.</div>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    className={`gap-2 transition-all ${voicePrescribing ? "bg-red-50 border-red-300 text-red-600 animate-pulse" : ""
                      }`}
